...
> **Latest:** v4.1.0 (2026-05-27) — **Vault-backed harness state.** Per-project harness state (`PLAN.md`, `progress.md`, `ROADMAP-*.md`, `FOLLOWUPS.md`, archived plans, designs/, features.json, init.sh, cursor files) relocates from `<project>/.harness/` to `<vault>/projects/<slug>/_harness/`. Vault folder `personal-projects/` renames to `projects/` in the same release. Backward-compat preserved: legacy paths still read via the resolver chain's tier-2 fallback with a one-warn-per-session-per-file notice; `.project-mode=local` is the operator-opt-out escape hatch. Migration is operator-paced via `scripts/migrate-harness-to-vault.sh <project>`; reversible via `--rollback`. Single-repo release (Crickets unaffected at v2.0.0). Per ROADMAP-V4 item #26.  
...
Currently shipping **v4.1.0** — vault-backed harness state. Per-project state (PLAN.md / progress.md / ROADMAP-*.md / FOLLOWUPS.md / archives / designs / features.json / init.sh / cursor files) lives at `<vault>/projects/<slug>/_harness/` post-migration; legacy `<project>/.harness/<file>` paths continue working via the resolver chain's backward-compat tier-2 fallback with a one-warn-per-session-per-file notice until the hard-cut deprecation release (deferred to a later v4.x). Crickets stays at v2.0.0 (no paired release). See [CHANGELOG.md](CHANGELOG.md) and the [latest release](https://github.com/alexherrero/agentm/releases/latest).
