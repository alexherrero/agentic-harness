...
> **Latest:** v4.13.0 (2026-06-01) — **Auto-orchestration: the memory push-surface** (V4 #23; single-repo MINOR). The Agent M memory skills were a *pull surface* — you had to remember to run recall, reflect, discover-skills, adapt-skills, and the watchlist by hand. This release makes them a **push surface**: a SessionStart **briefing** surfaces what's piled up (inbox over threshold · HIGH skill-watchlist · incubator ideas · GC-eligible ledger items) in one tight block; an **idle-time chain** runs reflect-corpus → discover-skills → adapt Pass-1 itself, detached and bounded; **phase-integration** reflects after `/work` (dedup-guarded vs the Stop hook, cross-host) and refreshes skill sources after `/release`; and two **nudges** suggest `/memory promote` for a recurring idea and flag stale `promoted` watchlist entries. It never blocks a session, never nags (cooldowns + a shifted-since-last-shown guard), and never acts on its own — every adoption stays operator-gated. Entirely **agentm-native** (DC-3) and hook/file-based + cross-host (DC-1). The last open V4 item — the foundation finish. 84 new tests (549 suite, 4-OS); three adversarial-review defects fixed. Per ROADMAP-V4 item #23.  
> **Prior:** v4.12.0 (2026-06-01) — **Cross-surface Agent M vault access** (V4 #22; single-repo MINOR). Your AgentMemory vault became readable natively from **every agent surface you use**, not just Claude Code — one paste-anywhere **context payload** (`templates/agentmemory-context.md`); **Claude.ai** (Google Drive connector) + **Claude Desktop** (filesystem MCP server) read it **read-only**; **Antigravity** gets it as an installed rule per-project (`.agents/rules/`) *and* globally (`~/.gemini/GEMINI.md`). Read/write **surface-scoped**. Per ROADMAP-V4 item #22.  
