...
You are running the `migrate-to-diataxis` skill. Canonical spec: `harness/skills/migrate-to-diataxis.md`. Convention: `harness/documentation.md`. Decision: ADR 0004 (`wiki/explanation/decisions/0004-diataxis-documentation-spec.md`).
...
**What this skill does not do:** rewrite page content, commit, push, delete files, touch anything outside `wiki/`, or auto-split mode-mixed pages (flagged for human review).

## Preconditions (check first, abort if not met)

1. Working tree clean: `git status --porcelain` empty.
2. `wiki/` exists at repo root.
3. `wiki/.diataxis` does **not** exist. If it does: exit with "already migrated".
4. At least one of `wiki/development/`, `wiki/operational/`, `wiki/design/`, `wiki/architecture/` exists. If none: exit with guidance.
5. Running from target project root, not agentic-harness repo.

## Input handling

- No argument → classify, preview, confirm, execute.
- `--preview` → classify and preview only; exit 0.
- `--yes` → skip confirmation prompt; preview still prints.

## Workflow

### 1. Walk and classify every `wiki/**/*.md`

Skip `Home.md`, `_Sidebar.md`, any `README.md`, and `.diataxis`. Apply rules in order; stop at first match:

| Rule | Pattern | Target |
|---|---|---|
| ADR | H1 matches `^# ADR \d{4}:` OR path under `architecture/decisions/`. | `explanation/decisions/` (preserve `NNNN`). |
| Status page | `> [!NOTE]` block containing `**Status:**` + `**Plan:**`. | `explanation/` |
...
| Tutorial | `## Step N —` headings AND `## What you learned` AND `## Next`. | `tutorials/` (preserve or add `NN-`). |
...
Old path → new path (basename preserved; ADRs keep `NNNN-` prefix). Scan every `wiki/**/*.md` for links to old basename / URL; build replacement table. Wiki-internal links use basenames only (old subdir dropped). Cross-mode basename collision → abort.
...
Prompt `Apply? [y/N]`. Only `y` / `yes` proceeds. Under `--yes` skip prompt.

### 5. Execute (in order)

1. `git mv` each page (not `mv` + `git add` — git must detect the rename).
2. Rewrite wiki-internal links per replacement table; one atomic edit per file.
3. Update `wiki/Home.md` + `wiki/_Sidebar.md` to reader-journey ordering (📚 → 🔧 → 📖 → 💡).
4. `rmdir` now-empty old dirs (fails if not empty — safety check; abort if so).
...
- Commit or push. Leaves staged-but-uncommitted.
- Delete content — `rmdir` on empty dirs only.
...
- Fail silently — every abort prints one-line reason and exits non-zero.
