...
|---|---|---|---|
| 2026-05-24 | [v3.0.1](https://github.com/alexherrero/agentic-harness/releases/tag/v3.0.1) — Agent M logo hero + brand asset set (harness-only PATCH) | **First visual brand iteration** — adds the Agent M logo asset set (~41 files across `assets/agent-m/` + `assets/m-monogram/` + `assets/index.html` brand-preview page; designed in Claude.ai Artifacts with brand palette `--ink: #0a0a0a` + `--paper: #f4efe6` and typography Inter Tight + JetBrains Mono) and refreshes `README.md` with a centered logo hero (`agent-m-clean-transparent-512.png` at displayed 256px) + italic tagline ("Persistent agentic memory + phase-gated engineering harness.") + reorganized badge layout into two centered blocks per the new [[personal-comms-style]] public-surface conventions. No behavior changes. **Solo harness PATCH** — not paired with Cricket; Cricket assets land separately when designed. Plan #15 task 1 close-out (Wave 1 of README refresh). | 2 commits (`v3.0.0..v3.0.1`); first visual asset commit since repo inception; establishes `assets/` as the brand-asset convention going forward |
| 2026-05-24 | [v3.0.0](https://github.com/alexherrero/agentic-harness/releases/tag/v3.0.0) — Agent M V3 close-out (paired with toolkit v1.0.0 — Cricket 1.0) | **Agent M V3 ships** — harness version now matches the memory implementation V-versioning (V3 = merged-Obsidian-and-GDrive vault with auto-recall in every harness phase + controlled write under the permeable A3 boundary). System renamed in operator-facing prose: **Agent M** (this repo + Cricket sibling toolkit + AgentMemory vault folder, together) + **Cricket** (the toolkit — noisy-cricket MiB ref; small primitives that punch above their weight). README rewrites (both repos) lead with brand framing; harness wiki Home rebranded; new HLD at `agent-toolkit/wiki/explanation/designs/agent-memory-evolution.md` walks V1→V4 evolution; new V3 retrospective at `agent-toolkit/wiki/explanation/v3-retrospective.md` covers what shipped / what we learned / what's deferred. Roadmap split into slim `ROADMAP.md` (non-V4 backlog) + new `ROADMAP-AgentMemoryV4.md` (9 V4-line items) + full pre-split snapshot at `ROADMAP.archive.20260523-v3-complete.md` (all operator-local; `.harness/` is gitignored). Folds in ROADMAP items #12 (retrospective) + #27 (Agent M evolution design doc) + #31 (META split-the-roadmap). | 2 commits (`v2.6.1..v3.0.0`); paired with [agent-toolkit v1.0.0](https://github.com/alexherrero/agent-toolkit/releases/tag/v1.0.0) which ships Cricket 1.0 (stable public API surface commitment: manifest schema + installer flags + bundles namespace + 11 customization kinds + per-host paths) + 4 commits on toolkit side (V3 retrospective + Agent M HLD + cross-ref fix + Cricket README/Home rewrite); closes ROADMAP items #12/#27/#31; **8th consecutive paired-release pair + first MAJOR-MAJOR pair**; harness V-versioning now matches memory implementation V-versioning explicitly |
| 2026-05-23 | [v2.6.1](https://github.com/alexherrero/agentic-harness/releases/tag/v2.6.1) — quality-gates bundle (paired with toolkit v0.13.0) | **One-command install for the 4 base operator-control + verification primitives** most `/work` sessions want: evaluator + kill-switch + steer + commit-on-stop + evidence-tracker. Paired-doc-only on harness side; substance ships in toolkit. Closes the "I forgot to install commit-on-stop and lost an hour" failure mode by offering `--bundle quality-gates` as a single adoption command. Bundle uses sibling-reference dispatch (bundle dir is just `bundle.md`; installer resolves to standalone primitives at install time) — net -1992 lines vs. the originally-planned COPY pattern after operator pushback mid-plan. | 1 commit (`v2.6.0..v2.6.1`); paired with [agent-toolkit v0.13.0](https://github.com/alexherrero/agent-toolkit/releases/tag/v0.13.0) which does the 6-commit sweep across plan #10 tasks 1-4 (bundle scaffold + sibling-reference pivot + smoke tests + Windows fixes + how-to + ADR 0010 + release); decision rationale + 2 locked design calls Q1-Q2 + 4 load-bearing assumptions in toolkit-side [ADR 0010](https://github.com/alexherrero/agent-toolkit/blob/main/wiki/explanation/decisions/0010-quality-gates-bundle.md); closes ROADMAP item #10; 7th consecutive paired-release pair; 2 cross-platform Python gotchas caught + fixed mid-plan (pwsh mkdir; inline-Python UTF-8) |
